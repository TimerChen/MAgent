...
from magent.builtin.tf_model.base import TFBaseModel
from magent.builtin.common import ReplayBuffer


class DeepQNetwork_meanh(TFBaseModel):
    def __init__(self, env, handle, name,
        ...
        custom_view_space: tuple
            customized feature space
        """
        TFBaseModel.__init__(self, env, handle, name, "tfdqn_mean_h")
        # ======================== set config  ========================
        self.env = env
        self.handle = handle
        ...
        self.input_all_view    = tf.placeholder(tf.float32, (None,self.mean_num,) + self.view_space)
        self.input_all_feature = tf.placeholder(tf.float32, (None,self.mean_num,) + self.feature_space)
        self.input_all_num = tf.placeholder(tf.float32, (None,1))
        ...
        with tf.variable_scope(self.name):
            with tf.variable_scope("eval_net_scope"):
                ...
                self.qvalues = self._create_network(self.input_view, self.input_feature,
                                                    self.input_all_view, self.input_all_feature,
                                                    use_conv)
    ...
    def _create_h_network(self, input_all_view, input_all_feature, input_all_num, reuse):
        h_size = [self.h_size]

        kernel_num = [32, 32]
        hidden_size = [128]

        input_all_view = tf.reshape(input_all_view, shape=(-1,) + self.view_space)
        input_all_feature = tf.reshape(input_all_feature, shape=(-1,) + self.feature_space)

        h_conv1 = tf.layers.conv2d(input_all_view, filters=kernel_num[0], kernel_size=3,
                                   activation=tf.nn.relu, name="h_conv1", reuse=reuse)
        h_conv2 = tf.layers.conv2d(h_conv1, filters=kernel_num[1], kernel_size=3,
                                   activation=tf.nn.relu, name="h_conv2", reuse=reuse)
        ...
        h_view = tf.layers.dense(flatten_view, units=hidden_size[0], activation=tf.nn.relu,
                                 name="h_dense_view", reuse=reuse)

        h_emb = tf.layers.dense(input_all_feature,  units=hidden_size[0], activation=tf.nn.relu,
                                name="h_dense_emb", reuse=reuse)

        dense = tf.concat([h_view, h_emb], axis=1)

        h_layer = tf.layers.dense(dense, units=self.mean_num, name="h_value", reuse=reuse)

        h_layer_shape = (-1,self.mean_num,)+tuple(h_layer.get_shape()[1:])
        h_layer = tf.reshape(h_layer, h_layer_shape)

        h_mean_layer = tf.reduce_sum(h_layer, axis=1) / input_all_num

        return h_mean_layer

    def _create_network(self, input_view, input_feature, input_all_view, input_all_feature, input_all_num,
                        use_conv=True, reuse=None):
        ...
        input_feature: tf.tensor
            the input tensor
        """
        self._create_h_network(input_all_view, input_all_feature, input_all_num, reuse)
        h_size = [self.h_size]
    ...
    def infer_action(self, raw_obs, all_obs, all_num, ids, policy='e_greedy', eps=0):
        ...
        if self.num_gpu > 1 and n > batch_size:   # infer by multi gpu in parallel
            ...
            for i in range(0, n, batch_size):
                ...
                ret.append(self.sess.run(self.output_action, feed_dict={
                    self.input_all_view: all_obs[0][beg:end],
                    self.input_all_feature: all_obs[1][beg:end],
                    self.input_all_num: all_num[beg:end],
    ...
    def _calc_target(self, next_view, next_feature, next_all_view, next_all_feature, next_all_nums,
                     rewards, terminal):
        ...
        if self.use_double:
            t_qvalues, qvalues = self.sess.run([self.target_qvalues, self.qvalues],
                                               feed_dict={
                                                   self.input_all_view: next_all_view,
                                                   self.input_all_feature: next_all_feature,
                                                   self.input_all_num: next_all_nums,
                                                   self.input_view: next_view,
                                                   self.input_feature: next_feature})
            next_value = t_qvalues[np.arange(n), np.argmax(qvalues, axis=1)]
        else:
            t_qvalues = self.sess.run(self.target_qvalues, {
                                                            self.input_all_view: next_all_view,
                                                            self.input_all_feature: next_all_feature,
                                                            self.input_all_num: next_all_nums,
                                                            self.input_view: next_view,
    ...
    def _add_to_replay_buffer(self, sample_buffer):
        ...
        for episode in sample_buffer.episodes():
            ...
            av, af, an = episode.all_views, episode.all_features, episode.all_nums
            av = np.array(av)
            af = np.array(af)
            an = np.array(an)
            m = len(r)
            ...
            self.replay_buf_all_view.put(av)
            self.replay_buf_all_feature.put(af)
            self.replay_buf_all_num.put(an)
    ...
    def train(self, sample_buffer, print_every=1000):
        ...
        for i in range(n_batches):
            ...
            batch_all_view = self.replay_buf_all_view.get(index)
            batch_all_feature = self.replay_buf_all_feature.get(index)
            batch_all_num = self.replay_buf_all_num.get(index)
            ...
            batch_next_all_num = self.replay_buf_all_num.get(index+1)
            batch_next_all_view = self.replay_buf_all_view.get(index+1)
            batch_next_all_feature = self.replay_buf_all_feature.get(index+1)

            batch_target = self._calc_target(batch_next_view, batch_next_feature,
                                             batch_next_all_view, batch_next_all_feature,
                                             batch_next_all_num,
                                             batch_reward, batch_terminal)

            ret = self.sess.run([self.train_op, self.loss], feed_dict={
                self.input_all_view:batch_all_view,
                self.input_all_feature:batch_all_feature,
                self.input_all_num: batch_all_num,
